...
from methods.Function_Integration.Integration_NewtonCotes_Func import newton_cotes_integration_func
...
A = 0.0
B = 1.0
DEGREE = 4 # Boole's Rule
EPSILON = 1e-6

def run():
    method_name = "Integration_NewtonCotes_Func"
    output_dir = os.path.join(os.path.dirname(__file__), '../..', 'output', method_name)
    os.makedirs(output_dir, exist_ok=True)
    
    info = (
        f"Phương pháp: Newton-Cotes (Degree={DEGREE}) (Function Mode)\n"
    ...
    result_dict = newton_cotes_integration_func(f, A, B, degree=DEGREE, epsilon=EPSILON, N_start=2)
    # N_start here means number of segments. Total points = N_start * degree
    ...
    t_hist.add_column("N_total") # Total intervals
    t_hist.add_column("h")
